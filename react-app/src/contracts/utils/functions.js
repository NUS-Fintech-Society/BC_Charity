...
export async function getWalletAddress(web3) {
  ...
  if (!window.ethereum || !web3 || !web3.eth) {
    alert(
      "This action requires ether but Metamask or other wallet providers are not enabled."
    );
  } else {
...
export function onboardCharity(adminAddress, UEN, sendFrom, networkID, web3) {
  const onboardingContract = new web3.eth.Contract(
    OnboardingJSON.abi,
    CharityChainJSON.networks[networkID].address
  );
  return onboardingContract.methods
    .onboardCharity(adminAddress, UEN)
    .send({ from: sendFrom });
}
...
export function getCharityAddress(UEN, networkID, web3) {
  const onboardingContract = new web3.eth.Contract(
    OnboardingJSON.abi,
    OnboardingJSON.networks[networkID].address
  );
 ...
 * @param {Web3} web3 Web3 instance
 */
export function addUserDonation(
  nricHash,
  amount,
  date,
  message,
  ...
  return charityChainContract.methods
    .addDonation(nricHash, amount, date, message)
    .send({ from: sendFrom });
}
...
export async function getUserDonations(nricHash, charityContractAddress, web3) {
  ...
  const donationCount = await charityChainContract.methods
    .getDonationCount(nricHash)
    .call();

  var donations = [];
  for (let i = 0; i < donationCount; i++) {
    const donationResult = await charityChainContract.methods
      .getDonation(nricHash, i)
      .call();
    const donation = {
 ...
 * @param {Web3} web3 Web3 instance
 */
export async function addContractOwner(
  walletAddress,
  contract,
  sendFrom,
  web3
...
export async function addAllContractOwner(walletAddress, sendFrom, web3) {
  charities.charities.forEach((charity) => {
    ...
    charityChainContract.methods
      .addOwner(walletAddress)
      .send({ from: sendFrom });
  });
}
